...
exceptions = ["XSS", "SQLi", "CSRF", "RCE", "RXSS"]  
...
def preprocess(df):
    ...
    #df['description'] = df['description'].apply(lambda text: " ".join([word if word in exceptions else lemmatizer.lemmatize(word.lower()) for word in word_tokenize(text) if not word.lower() in stop_words and word not in string.punctuation]))
    df['description'] = df['description'].str.lstrip()
    def lemmatize_text(text):
        tokens = word_tokenize(text)
        pos_tags = nltk.pos_tag(tokens)
        lemmatized_tokens = [word if word in exceptions else lemmatizer.lemmatize(word, get_wordnet_pos(pos)) for word, pos in pos_tags if not word.lower() in stop_words and word not in string.punctuation]
        return " ".join(lemmatized_tokens)

    df['description'] = df['description'].apply(lemmatize_text)    
    df['description'] = df['description'].apply(lambda s: re.sub(r'\[.*?\]', '', s))
    df['description'] = df['description'].apply(lambda s: re.sub(r"(?i)cve[-_\s]?\d{4}[-_\s]?\d{1,5}\*?'?\)?", '', s))
    df['description'] = df['description'].apply(lambda s: re.sub(r'^-', '', s).strip())
    df['description'] = df['description'].apply(lambda s: re.sub(r'^:', '', s).strip())
    df['description'] = df['description'].apply(lambda s: re.sub(r'\s\(GHSL-\d{4}-\d{3}\)', '', s))
    ...
    df['description'] = df['description'].astype(str).apply(lambda x: re.sub(pattern, '', x))
...
def main():
    ...
    if re.search(r'base\.xlsx$', file_path):
        columns_to_check = ['description', 'vuln_type', 'severity_rating']
    else:
        columns_to_check = ['description', 'vuln_type', 'severity_rating', 'score', 'vector']
